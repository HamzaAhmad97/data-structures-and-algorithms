...
const validateWord = (word) => /[a-zA-Z]{5,10}/.test(word) && !/\w{11}/.test(word);
...
const validateEmail = (email) => /^\w+(.\w+)?@[a-zA-Z]+(.net|.com|.org)$/.test(email);
...
const validatePhoneNumber = (phoneNumber) => /^(\(?\d{3}\)?)[-\W]?(\d{3}[-\W]?\d{4})$/.test(phoneNumber) && !/\(\d{3}(?!\))/.test(phoneNumber); 
