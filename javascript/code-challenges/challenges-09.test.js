...
E.g. [4,2,7,5,9,2] -> 9
------------------------------------------------------------------------------------------------ */
const maxInArray = arr => arr.reduce((tot, itm) => tot = (Number(itm) > tot ? Number(itm) : tot),0);
...
const updateNumbers = obj => Object.keys(obj).map((itm,i) => `${itm}: ${Object.values(obj)[i]}`);
...
const getHouses = arr => arr.map(itm => itm.house);
...
const hasChildrenValues = (arr, character) => arr.filter(itm => itm.name === character ? itm.hasOwnProperty('children') ? true: false: false).length !== 0 ? true: false;
...
const hasChildrenEntries = (arr, character) => hasChildrenValues(arr,character);
...
const totalCharacters = arr => arr.reduce((tot, itm) => tot+1+ (itm.hasOwnProperty('children')? itm.children.length:0) + (itm.spouse? 1 : 0),0);
/* ------------------------------------------------------------------------------------------------
...
const houseSize = arr => arr.map(itm => ({house: itm.house, members: 1 + (itm.hasOwnProperty('children') ? itm.children.length : 0) + (itm.spouse? 1: 0)}));
...
const houseSurvivors = arr => arr.map(itm => ({house: itm.house, members: 1 + (itm.hasOwnProperty('children') ? itm.children.length : 0) + (itm.spouse? deceasedSpouses.includes(itm.spouse)? 0: 1: 0)}));
