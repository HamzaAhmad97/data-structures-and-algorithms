...
const longestString = arr =>arr.length ? arr.map((itm,i) => [itm.length, i]).sort((a,b) => a[0] < b[0] ? 1 : a[0] > b[0] ? -1 : 0)[0][1] : -1;
...
const firstLetters = arr => arr.map(itm => itm.charAt(0));
...
const findHappiness = (arr) => arr.filter(itm => itm.includes(':)'));
...
const standardizePhoneNumbers = (arr) => arr.map(itm => `${itm.substring(1,4)}${itm.substring(6,9)}${itm.substring(10)}`);
...
const onlyOddChars = str => str.split('').filter(itm => str.indexOf(itm) % 2 !== 0).join('');
...
const allHappy = arr => arr.every(itm => itm.includes(':)'));
