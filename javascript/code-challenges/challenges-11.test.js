...
const transformToLis = obj => Object.keys(obj).map(itm => `<li>${itm}: ${obj[itm]}</li>`);
...
const count = (target, input) => input.reduce((tot, itm) =>tot + itm.reduce((tot, itm) => tot + (itm === target ?1:0) ,0),0);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const totalSum = input => input.reduce((tot, itm) => tot + itm.reduce((tot, itm) => tot + itm,0),0);
...
const divisibleByFiveTwoToThePower = input => input.map(itm => itm.filter(itm => typeof itm === 'number' && itm % 5 === 0 ).map(itm => 2**itm));
...
let findMaleAndFemale = data => data.filter(itm => itm.gender === 'male' || itm.gender === 'female' ).reduce((tot, itm) => tot + `${tot === '' ? '' : ' and'} ${itm.name}`,'').trim();
/* ------------------------------------------------------------------------------------------------
CHALLENGE 6 
...
let findShortest = data =>data.filter(itm => itm.height == data.map(itm => itm.height).map(itm => Number(itm))
.sort((a,b) => a < b ? -1 : a > b ? 1 : 0)[0])[0].name ;
