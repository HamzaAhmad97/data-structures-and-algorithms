...
------------------------------------------------------------------------------------------------ */
const toLastNames = people => people.map(({firstName, lastName}) => `${firstName} ${lastName}`);
...
const addValues = arr => arr.reduce((acc, itm) => acc + itm, 0) ;
...
const addPurchases = arr => arr.reduce((acc,{purchasePrice}) => acc+purchasePrice,0);
...
const returnNames = arr => arr.reduce((acc,{name}) => acc.concat(name),[]);
...
const reversedString = str => str.split('').sort((a,b) => -1).reduce((acc, itm) => acc.concat(itm), []).join('');
...
const countNumberOfChildren = arr => arr.filter(itm => itm.hasOwnProperty('children')).reduce((acc, {children}) => acc + children.length,0);
...
const calculateAverage = arr => arr.reduce((acc,itm) => acc + itm, 0)/arr.length;
/* ------------------------------------------------------------------------------------------------
...
const countPrimeNumbers = arr => arr.filter(itm => isPrime(itm)).reduce((acc) => acc + 1,0);
/* ------------------------------------------------------------------------------------------------
...
const extractStat = (statName, arr) => {
  let res = arr.reduce((acc, itm) =>
    statName === itm.stat.name ? acc.concat(itm) : acc,[]);
...
const extractChildren = arr => arr.filter(itm => itm.name.includes('a')).filter(itm => itm.hasOwnProperty('children')).reduce((acc, {children}) => acc.concat(children),[]);
