...
const screenForNames = (arr) => arr.filter(itm => /^(Mr.|Mrs.|Ms.|Dr.)\W\w+/.test(itm)).filter(itm => /[^0-9]/.test(itm));
...
const toTitleCase = (arr) => arr.map(itm => itm[0].toUpperCase() + itm.slice(1));
...
let biggerThanLuke = (arr) => arr.filter(itm => Number(itm.mass) > 77).reduce((tot, itm) => tot + (tot === '' ? '' : ' - ' ) + itm.name,``);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
...
const sortBy = (property, arr) => arr.sort((a,b) => a[property] > b[property] ? 1 : a[property] < b[property] ? -1 : 0 );
...
const detectTicTacToeWin = (board) => {
  let val = {X:[],O:[] };
  ['X', 'O'].forEach(a => board.forEach((b,k) => b.forEach((c,i) => c === 'X' ? val.X.push(i + 3 * k) : c === 'O' ? val.O.push(i + 3 * k) : 0)));
  console.log(val);
}; 
