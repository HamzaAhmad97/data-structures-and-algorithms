...
const raisedToTheThird = (arr) => {
  // Solution code here...
  let temp = [];
  arr.forEach(item => temp.push(Math.pow(item,3)));
  return temp;
};
...
const forEachTwoToThe = (arr) => {
  // Solution code here...
  let temp =[];
  arr.forEach(item => temp.push(Math.pow(2,item)));
  return temp;
};
...
const charCode = arr => arr.map(item => item.charCodeAt(0));
...
const evenOdd = (arr) => arr.map(item => typeof item === 'number' ? item % 2 === 0 ? 'even' : 'odd' : 'N/A');
...
const extractStats = arr => arr.map(({stat,effort, baseStat}) => ({name: stat.name, total: effort+baseStat}));
