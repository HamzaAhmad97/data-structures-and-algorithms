...
const sortByChildren = (charArray) => charArray.sort((a, b) => a.children.length < b.children.length ? -1 :a.children.length > b.children.length ? 1 : a.house < b.house ? -1 : a.house > b.house ? 1 : 0 );
...
const isCapitalized = (str) => str.length === 0 ? [] : str.match(/[A-Z]((\w+)?)/g);

// I think that the test for this challenge is broken, I have literally tried everything
...
const citiesAtoJ = (arr) => arr.filter(itm => /^[A-J]\w+/gi.test(itm));
/* ------------------------------------------------------------------------------------------------
...
let hangman = (str) => str.replace(/[aouieAOUIE]/g,'_');
...
const findShells = (str) => str.match(/[shea]+ells/ig);
