...
const getNames = arr => arr.map(itm => itm.name.split('').reverse().join(''));
// destructuring is not working for some reason
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const appendTheEnd = str => str.slice().concat(' The end.');
...
const setStatusAsAuthor = people => people.map(itm => itm.isAuthor = true);
